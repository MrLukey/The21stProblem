...
{
    const all_ref_data = await getJSONViaAJAX('app/assets/references.json').then(ref => ref['references']);
    all_ref_data.forEach(ref => {
        ref['has_image'] = (ref['image_url'] !== '');
    })

    const hidden_ref_hb_template = await getTextViaAJAX('app/src/js/templates/reference_tooltip.hbs');
    ...
    const full_ref_hb_template = await getTextViaAJAX('app/src/js/templates/full_reference.hbs');
    const full_ref_template = Handlebars.compile(full_ref_hb_template);
    populate_references(all_ref_data, full_ref_template);

    const all_info_graphic_data = await getJSONViaAJAX('app/assets/info_graphics.json')
    ...
    const info_graphic_hb_template = await getTextViaAJAX('app/src/js/templates/info_graphic.hbs');
