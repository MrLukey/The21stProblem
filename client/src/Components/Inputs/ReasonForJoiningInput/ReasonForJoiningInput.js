...
const ReasonForJoiningInput = (props) => {
    ...
    const handleReasonForJoiningInput = (evt) => {
        ...
        if (reasonForJoining.length > 0 && reasonForJoining.length <= 500){
            props.setReasonValid(' is-valid')
        } else {
            props.setReasonValid('')
        }
    }

    return (
        <FormGroup className="form-floating mx-1 mb-2" >
            <textarea className={"form-control h-auto" + props.reasonValid}  rows={5} id="comment"
                      value={props.reasonForJoining} onChange={handleReasonForJoiningInput} maxLength={500} />
